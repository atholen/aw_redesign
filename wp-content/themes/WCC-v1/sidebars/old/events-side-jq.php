...
<script type = "text/javascript">
	$(function(){
		var eventArray = <?php echo $jsonArray ?>;
		var i = 0;
		
		$('h3#show-more').click(function(){
		
			title = eventArray[i]['title'];
			permalink = eventArray[i]['permalink'];
			the_date = eventArray[i]['the_date'];
			venue = eventArray[i]['venue'];
			
	
			$('<li><a href="'+ permalink +'"><div class="info"><span class="time">'+ the_date + '</span><span class="venue">' + venue + '</span></div><div class = "content"><strong>' + title + '</strong></div><div style="clear:both;"></div></a></li>').hide().appendTo('ul.events').slideDown();
